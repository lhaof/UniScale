...
def parse_args():
    ...
    parser.add_argument('--config', help='train config file path',
                         default="/mnt/bn/hjj-humanseg-yg/UniScaleFormer/Mask2Former/mmdetection/projects/MultiScaleMask2Former/configs/KFOLD_UniScaleFormer/SwinB_FOLD_3.py")
    parser.add_argument(
        '--num-images',
        type=int,
        default=50,
...
def inference(args, logger):
    ...
    classes = ['Neoplastic', 'Inflammatory', 'Connective', 'Dead', 'Epithelial', 'Gland', 'Malignant Lesion', 'Proximal Tubular', 'Distal Tubular', 'Nuclei', 'Intercellular', 'background']
    if  hasattr(model.panoptic_head, "language_encoder"):
        model.panoptic_head.language_encoder.get_text_embeddings(classes, is_eval=True)
    ...
    for idx, data_batch in enumerate(data_loader):
        ...
        # avg_flops.append(flops.total())
        start = time.perf_counter()
        outputs = model(data['inputs'])
        end = time.perf_counter()
        total_time += end - start

    fps = args.num_images / total_time
    print(f'FPS: {fps:.3f}')

    # import pdb; pdb.set_trace()

    mean_flops = (np.average(avg_flops)) / 1e9
    result['flops'] = mean_flops
    print(f'flops: {mean_flops}')
    # result['params'] = params

        # outputs = get_model_complexity_info( model,
        #     None,
        #     inputs=data['inputs'],
        #     show_table=True,
        #     show_arch=False)
