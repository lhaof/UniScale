...
class AuxScaleMask2FormerHead(MaskFormerHead):
    ...
    def __init__(self,
        ...
        if self.multi_head:
            self.cls_embed = nn.ModuleList([nn.Linear(feat_channels, self.num_classes + 1) for _ in range(scale_cls)])
        ...
        if self.auxscaleloss:
            ...
            self.input_proj = ConvModule(
                ...
                bias=True)
            if version == "v2":
                self.scale_decoder = ScaleAwareModule(**transformer_decoder.layer_cfg)
            elif version == "v1":
                self.scale_decoder = Mask2FormerTransformerDecoder(num_layers=1, layer_cfg=transformer_decoder.layer_cfg, return_intermediate=False)
    ...
    def loss(
        ...
        if self.auxscaleloss:
            ...
            loss_scale = self.loss_scale(scale_decoder_pred.flatten(0, 1), scale_gt)
        else:
