...
class MaskFormerFusionHead(BasePanopticFusionHead):
    ...
    def panoptic_postprocess(self, mask_cls: Tensor,
        ...
        """
        object_mask_thr = self.test_cfg.get('object_mask_thr', 0.7)
        ...
        if H * W > 1048 * 1048:
            # process in gpu
            cur_classes = cur_classes.cuda()
            cur_masks = cur_masks.cuda()
            cur_prob_masks = cur_prob_masks.cuda()
    ...
    def instance_postprocess(self, mask_cls: Tensor,
        ...
        if H*W > 2048 * 2048:
            ...
            scores_per_image = scores_per_image.cpu()
            is_thing = is_thing.cpu()
        ...
        if H*W > 2048 * 2048:
            # process in gpu
            mask_pred_binary = mask_pred_binary.cuda()
            bboxes = bboxes.cuda()
            det_scores = det_scores.cuda()
