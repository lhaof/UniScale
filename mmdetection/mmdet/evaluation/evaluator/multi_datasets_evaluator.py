...
class MultiDatasetsEvaluator(Evaluator):
    ...
    def evaluate(self, size: int) -> dict:
        ...
        metrics_results['Avg_PQ'] = 0
        metrics_results['Avg_AJI'] = 0
        metrics_results['Avg_DICE'] = 0
        for dataset_prefix, start, end, metric in zip(
            ...
            if is_main_process():
                ...
                print_log(f'================{final_prefix}================', logger=logger)
                _print_nuclei_metrics(_metrics)
                print_log(f'================{final_prefix}================', logger=logger)
                # import pdb; pdb.set_trace()
                metrics_results["Avg_PQ"] += _metrics["PQ"]
                metrics_results["Avg_AJI"] += _metrics["AJI"]
                metrics_results["Avg_DICE"] += _metrics["DICE"]
                metric_results = {
        ...
        if is_main_process():
            metrics_results["Avg_PQ"] /= len(self.dataset_prefixes)
            metrics_results["Avg_AJI"] /= len(self.dataset_prefixes)
            metrics_results["Avg_DICE"] /= len(self.dataset_prefixes)
            print_log(f'================Average Metric================', logger=logger)
            _print_nuclei_metrics(metrics_results, "Avg_PQ", "Avg_AJI", "Avg_DICE")
            print_log(f'================Average Metric================', logger=logger)
...
def _print_nuclei_metrics(nuclei_stat, PQ="PQ", AJI="AJI", DICE="DICE"):
    headers = ["", PQ, AJI, DICE]
    data = [["Nuclei Segmentation"] + [nuclei_stat[PQ]] + [nuclei_stat[AJI]] + [nuclei_stat[DICE]]]
    table = tabulate(
        data, headers=headers, tablefmt="pipe", floatfmt=".3f", stralign="center", numalign="center"
    )
    print("Nuclei segmentation metrics:\n" + table)